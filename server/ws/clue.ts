...
import { getOrCreateRoom, broadcastToAll, rooms } from './rooms'
...
const ADJACENCY: Record<RoomId, RoomId[]> = {
  study:         ['library', 'kitchen'],        // kitchen = secret passage
  library:       ['study', 'billiard_room', 'ballroom'],
  billiard_room: ['library', 'conservatory', 'dining_room'],
  kitchen:       ['study', 'ballroom'],          // study = secret passage
  ballroom:      ['kitchen', 'library', 'conservatory', 'lounge'],
  conservatory:  ['billiard_room', 'ballroom', 'lounge'], // lounge = secret passage
  hall:          ['study', 'lounge', 'dining_room'],
  lounge:        ['hall', 'ballroom', 'conservatory', 'dining_room'], // conservatory = secret passage
  dining_room:   ['billiard_room', 'hall', 'lounge'],
}
...
function startGame(state: RoomState): void {
  ...
  // Cards that couldn't be distributed evenly are public
...
function handleSuggest(
  ...
  // Move named suspect's piece
  for (const p of state.players.values()) {
    // We track suspect positions separately but for simplicity we broadcast suspect_moved
  }

  // Build disprove order (clockwise from suggester, excluding them)
...
function askNextDisprover(state: RoomState): void {
  ...
  if (suggestion.disprovingSlot >= suggestion.disproveOrder.length) {
    // Nobody could disprove — tell suggester
...
function handleDisprove(state: RoomState, player: CluePlayer, card: CardId): void {
  ...
  // Tell the suggester privately what card was shown
  const suggester = state.players.get(suggestion.byUserId)!
  sendTo(suggester, {
  ...
  // Tell everyone a card was shown (not which one)
  broadcastToAll(state.roomId, {
...
function handleAccuse(
  ...
  if (correct) {
    ...
    // Wrong accusation — player is eliminated but game continues
    player.alive = false
    broadcastToAll(state.roomId, {
      type: 'ACCUSATION_RESULT',
      userId: player.userId,
...
export default defineWebSocketHandler({
  ...
  async message(peer, message) {
    ...
    if (type === 'READY') {
      ...
      // Start when at least 3 players have joined
